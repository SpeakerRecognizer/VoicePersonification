...
В мире искусственного интеллекта господствуют большие языковые модели (LLM). GPT и ее аналоги прекрасно справляются с написанием текстов, кода и генерацией картинок. Но что насчёт звука? Умение слушать и понимать аудио — это следующий логический шаг на пути к многомодальным системам.
...
#### **Особенности задачи**
В отличие от текста, аудио имеет непрерывную структуру. Кроме того, полезная информация в аудиозаписях тесно переплетена с паразитной. Так, слова и содержание аудиосообщения могут искажаться шумом, эмоциями и индивидуальными характеристиками голоса, что осложняет задачу. 

Существующие подходы, такие как **SALMONN** [4] и **Qwen2-Audio** [5], пытаются работать со звуком, используя специальные предобученные энкодеры (например, Whisper), которые преобразуют аудио в векторные представления. Но, как показывает практика, подобные модели часто страдают от переобучения, и не способны обобщать свои возможности на различные аудио-сценарии.
Наша цель — преодолеть это препятствие и создать модель, которая сможет одинаково уверенно работать с самыми разными аудио-задачами.
...
1.  **Аудио-энкодер:** За восприятие аудио в модели отвечает **WavLM** [2]. В отличие от моделей вроде Whisper, обученных в supervized-режиме, WavLM обучался в манере self-supervised на огромном массиве аудиоданных. Это, как показывает практика, должно давать ему более универсальное представление о звуках любого типа.
2.  **Коннектор:** Чтобы «перевести» выход WavLM'а на язык LLM, мы использовали трансформерный коннектор. Он выжимает из аудиопотока два типа представлений: **5 эмбеддингов на уровне всего предложения** (представления глобального контекста) и **последовательность фреймов с частотой 2.5 Гц** (представления, отражающие динамику во времени).
...
> **Архитектура Cryfish.** WavLM преобразует аудио в фичи, коннектор сжимает и адаптирует их для подачи в LLM, а Qwen2 генерирует ответ на основе мультимодального контекста.
